...
- 选项：-f表示持续跟踪；我们可以在FinalShell中复制标签，在两个标签里写命令，若被持续跟踪的文件有变化，那么将立刻体现；（Ctrl+C可以退出持续跟踪模式）
- 选项：-num表示查看尾部多少行，不写表示10行；
  
### 10.vi编辑器
vi\vim(visual interface) 是linux中最经典的文本编辑器；vi是命令行下对文本文件进行编辑的绝佳选择；

#### (1) 工作模式
- 命令模式(command mode)：命令模式下所敲的按键编辑器都理解为命令，以命令驱动执行不同的功能，此模式下不能自由进行文本编辑；
- 输入模式(insert mode)：在命令模式下以`i`等快捷键启动，此时可以对文件内容进行自由编辑；
- 底线命令模式(last line mode)：在命令模式下以`:`开始，通常用于文件的保存和退出；

#### (2)命令模式
如果要通过vi/vim编辑器编辑文件，则需要通过以下命令
```
vi 文件路径
vim 文件路径
```
- vim兼容全部的vi功能，后续全部使用`vim`命令；
- 如果文件路径表示的文件不存在，那么此命令会用于编辑新文件，否则编辑已有文件；
